...
7. When English (UK) spelling is chosen and the take is English, respell American forms the British way
   (`BritishSpellingConverter`, the macOS table). The person's own Custom Words keep their spelling. A Whisper
   take is English when Whisper says so. Parakeet reports no language, so choosing British is the
   statement that the person dictates English (the macOS "English (UK)" choice is a language lock too); the
   setting says so on screen, because the table shares "color" and "favor" with Spanish and Portuguese.
   A text-level language guess was tried and withdrawn: a word list cannot tell those languages apart.
...
| `RecordingActive` | `active => _pushToTalkHook?.SetRecordingActive(active)`. Called only by `RecordingFlag` (`App/Composition/SessionComposition.cs`), which follows the controller's `SessionChanged` - every ending, the watchdog's and the recovery's aborts included - and passes on changes only (#86). | One external operation (a flag on the hook). |
...
| `RecordTransition` `App/Composition/SessionEffects.cs:64` | One log line for the transition, plus the capture's open and stream timings on a start. It no longer sets the hook's flag: the aborts never pass it (#86). | Observation. The `switch` maps a transition kind to a log code; it chooses nothing about the session. | The transition was made by the controller under the executor's `switch` on the signal `Pipeline/DictationSessionExecutor.cs:526`. |
...
- `SessionEffects.RecordTransition` `App/Composition/SessionEffects.cs:64`: up to three log lines, selected by the kind of a transition already made. The hook's recording flag is updated separately by `RecordingFlag`, which follows the controller's `SessionChanged` event and forwards changes only.
