...
- **One owner of session transitions** (`DictationSessionCoordinator`, since #148 steps 1 and 11): a
  ...
  `DictationSessionExecutor.PreviewAbortDeadline`, three seconds; `LivePreviewAborted` on the log,
  the resource let go of once the exit is seen); one still not seen gone is recorded
  (`LivePreviewFailed` / `RuntimeResourceBusy`) and the transcription goes on regardless, because a
  preview failure must not fail the recording. Proved through the production composition
...
| --- | --- | --- | --- |
| `EscapeRecoveryEnabled` | `parts.Shell.Dictation().EscapeRecoveryEnabled`. | Observation. | Read once as a recording starts, kept as `_escapeRecoveryForSession` `Pipeline/DictationSessionExecutor.cs:544`; whether Escape releases or cancels is the executor's `Pipeline/DictationSessionExecutor.cs:517`. |
| `RecordResourcePressure` | One `AppLogEntry`. | Observation. | Admission: `SystemResourceAdmissionPolicy.Evaluate(_resources.Probe())` `Pipeline/DictationSessionExecutor.cs:493`. |
...
| `RecordPreviewStillRunning` `App/Composition/SessionEffects.cs:227` | One log line. | Observation. | Reading the release's stop report, ending a preview worker that refused its stop by force before the final transcription, and going on when it was not seen gone are the executor's (`Pipeline/DictationSessionExecutor.cs:625`). |
...
executor armed `Pipeline/DictationSessionExecutor.cs:621`
...
| Resource admission of a press (memory refuses, disk withholds the recovery copy) | `DictationSessionExecutor.ExecutePushToTalkAsync` `Pipeline/DictationSessionExecutor.cs:493` with `SystemResourceAdmissionPolicy` (Core) |
...
| Abort and reset (a failed or timed-out command; a stale terminal ignored) | `DictationSessionExecutor.RecoverFailedSessionAsync` `Pipeline/DictationSessionExecutor.cs:410`, `TimeOutAsync` `Pipeline/DictationSessionExecutor.cs:430`, `ForSession` check `Pipeline/DictationSessionExecutor.cs:477` |
| Finalisation invocation and its steps | `DictationSessionExecutor.FinalizeAsync` `Pipeline/DictationSessionExecutor.cs:614` → `SessionFinalizationRunner.RunAsync` `Pipeline/SessionFinalizationRunner.cs:205` |
