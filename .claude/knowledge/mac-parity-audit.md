...
| Ollama catalogue install and remove | **Built 2026-09-25, #213.** The AI Polish page guides setup, downloads the eleven curated macOS models with their dated macOS verdicts, shows progress per layer with Stop, and removes catalogue models after a confirmation. Driven on the test PC against Ollama 0.34.4: stop part way, a full download, a removal, every pre-existing model untouched. | **Deliberately different, with reasons:** hosted Ollama models are not offered (the Windows contract keeps Ollama on the device, and discovery already refuses remote models); the app never starts Ollama (macOS has a Start button): the Windows desktop app applies its own saved "expose to the network" setting, which the app cannot read reliably, and it can stop a server another tool started - so the page says to start it from the Start menu; no hidden `ollama serve`; a download runs on when the page is left or the window hidden, as a browser's does, and stops on a switch away from Ollama or the app's exit; removal asks first and is offered only for catalogue models and never for the model the running provider uses (macOS deletes on one click); no warm-up; no polling while the page is hidden. |
...
| Modifier-only hotkeys, `Ctrl+Win` as the default | #66. Built. Needs a real keyboard **for the modifier tap specifically**. Single non-modifier keys are now driven synthetically through the real hook (`--synthetic-hotkey`, PR #144, 108-109 ms press-to-recording), but a modifier-only tap completes on a 40 ms poll an instant press can fall between - a declared boundary in `uat-testing.md`. Founder decision 2026-09-05: **full binding control is the requirement; Ctrl+Win is the default** (matches WisprFlow and other competitors). |
