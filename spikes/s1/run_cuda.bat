@echo off
setlocal
rem S1 CUDA run: ORT 1.29 CUDA EP needs the local venv's NVIDIA DLL directories on PATH.
rem Usage: run_cuda.bat [--decoder fp32]
set "SPIKE_ROOT=%~dp0"
set "CUDA_VENV=%SPIKE_ROOT%venv-cuda"
set "PATH=%CUDA_VENV%\Lib\site-packages\nvidia\cu13\bin\x86_64;%CUDA_VENV%\Lib\site-packages\nvidia\cudnn\bin;%PATH%"
"%CUDA_VENV%\Scripts\python.exe" "%SPIKE_ROOT%s1_latency.py" --tier cuda %1 %2
endlocal
