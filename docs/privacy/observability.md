...
| `recognitionLanguage` | optional `DiagnosticRecognitionLanguage` enum | The language a take was recognised in as the engine was told it: Automatic, English, French, German, Spanish, or Other. It is the person's language setting, the same for every take until they change it, never what was said or detected. Absent for an engine that takes no language. |
| `snippetImport` | optional object: `source` and `outcome` enums, optional `failure` enum, six integers 0–100,000 | One snippet import attempt, as macOS reports it: where it came from (paste, a file kind, or Wispr Flow; never a file name), how it ended (completed, nothing found, nothing compatible, nothing approved, list changed, failed) with a fixed failure category, and counts of candidates, added, already had, listed twice, unticked, and left out by the source. Never a trigger, a snippet's text, a file path, or another app's content. Out-of-range values drop the whole object. |
| `wordImport` | optional object: `source` and `outcome` enums, optional `failure` enum, six integers 0–100,000 | One import of words from another dictation app: which app (Wispr Flow or Handy, a fixed list; never a path), how it ended (completed, nothing found, nothing compatible, nothing new, cancelled, list changed, failed) with a fixed failure category, and counts of pairs reviewed, added, already had, corrected differently, that could not be stored, and left out by the source. Never a word, a spelling, a file path, or another app's content. Out-of-range values drop the whole object. |
