...
3. Wait for the top-right pill to say `ready` and `hold F9 · GPU + EG-1`.
4. Click into a text field in Notepad, Chrome, Discord, or another app.
5. Hold **F9**, speak, then release **F9**.
...
The strict synthetic F9 and paste test was correctly blocked while Windows was locked:
...
- Transcription begins after F9 is released. Live streaming is not included yet.
- F9 is push-to-talk, not tap-to-toggle.
