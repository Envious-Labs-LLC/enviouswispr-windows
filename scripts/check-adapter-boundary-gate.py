...
MUTATIONS: dict[str, list[tuple[str, str]]] = {
    ...
    "pattern-erases-external": [(FOCUS_READ, "        object? identity = null;" + NL + "        _ = Automation(() => { if (element is object snapshot) { identity = snapshot; } return 0; });" + NL + "        _ = identity?.GetHashCode();" + NL + FOCUS_READ)],
    "switch-pattern-erases": [(FOCUS_READ, "        var identity = Automation(() => element switch { object snapshot => snapshot });" + NL + "        _ = identity.GetHashCode();" + NL + FOCUS_READ)],
    "foreach-object-variable-returned": [(CARET_READ, "        var children = Automation(() => { var copied = new List<object>(); foreach (object child in textPattern.DocumentRange.GetChildren()) { copied.Add(child); } return copied; });" + NL + "        foreach (var child in children)" + NL + "        {" + NL + "            _ = child.GetHashCode();" + NL + "        }" + NL + NL + CARET_READ)],
    "foreach-object-variable-external": [(CARET_READ, "        var copied = new List<object>();" + NL + "        _ = Automation(() => { foreach (object child in textPattern.DocumentRange.GetChildren()) { copied.Add(child); } return 0; });" + NL + "        _ = copied.Count > 0 ? copied[0].GetHashCode() : 0;" + NL + CARET_READ)],
    "array-copyto-returned-buffer": [(CARET_READ, "        var children = Automation(() => { var found = textPattern.DocumentRange.GetChildren(); var copied = new object[found.Length]; found.CopyTo(copied, 0); return copied; });" + NL + "        foreach (var child in children)" + NL + "        {" + NL + "            _ = child.GetHashCode();" + NL + "        }" + NL + NL + CARET_READ)],
    "array-copyto-external-buffer": [(CARET_READ, "        var copied = new object[8];" + NL + "        _ = Automation(() => { textPattern.DocumentRange.GetChildren().CopyTo(copied, 0); return 0; });" + NL + "        _ = copied[0]?.GetHashCode();" + NL + CARET_READ)],
    "array-clone-escapes": [(CARET_READ, "        var cloned = Automation(() => textPattern.DocumentRange.GetChildren().Clone());" + NL + "        _ = cloned.GetHashCode();" + NL + CARET_READ)],
